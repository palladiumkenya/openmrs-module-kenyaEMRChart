...
-- ------------- update etl_patient_sari-------------------------
DROP PROCEDURE IF EXISTS sp_update_etl_patient_sari $$
CREATE PROCEDURE sp_update_etl_patient_sari(IN last_update_time DATETIME)
  BEGIN
    ...
    ON DUPLICATE KEY UPDATE visit_date=VALUES(visit_date),encounter_provider=VALUES(encounter_provider),
     ...
     abnormal_breath_sounds=VALUES(abnormal_breath_sounds),wheezing=VALUES(wheezing),sore_throat=VALUES(sore_throat),difficulty_in_breathing=VALUES(difficulty_in_breathing),
     chest_pain=VALUES(chest_pain),rhinorrhea=VALUES(rhinorrhea),sore_muscles=VALUES(sore_muscles),haemoptysis=VALUES(haemoptysis),chills=VALUES(chills),diarrhea=VALUES(diarrhea),conjunctivitis=VALUES(conjunctivitis),convulsions=VALUES(convulsions),rigors=VALUES(rigors),pneumonia_clinical_diagnosis=VALUES(pneumonia_clinical_diagnosis),unable_to_breastfeed=VALUES(unable_to_breastfeed),vomits_everything=VALUES(vomits_everything),
     stridor=VALUES(stridor),grunting=VALUES(grunting),nasal_flaring=VALUES(nasal_flaring),chest_in_drawing=VALUES(chest_in_drawing), lethargic=VALUES(lethargic),unconscious_comatose=VALUES(unconscious_comatose),
     reported_lmp=VALUES(reported_lmp),pregnant=VALUES(pregnant),due_date=VALUES(due_date),child_born_at_term=VALUES(child_born_at_term), weeks_of_gestation=VALUES(weeks_of_gestation),chronic_respiratory_disease=VALUES(chronic_respiratory_disease),
     chronic_neurological=VALUES(chronic_neurological),prior_tb=VALUES(prior_tb),hiv_aids=VALUES(hiv_aids),newly_diagnosed_tb=VALUES(newly_diagnosed_tb), chronic_cardiac=VALUES(chronic_cardiac),chronic_liver_disease=VALUES(chronic_liver_disease),
     chronic_renal_disease=VALUES(chronic_renal_disease),malnutrition=VALUES(malnutrition),diabetes=VALUES(diabetes),asthma=VALUES(asthma),cancer=VALUES(cancer),sickle_cell_disease=VALUES(sickle_cell_disease),
     rickets=VALUES(rickets),covid_19=VALUES(covid_19),other_specify=VALUES(other_specify),existing_chronic_condition_in_past_3_months=VALUES(existing_chronic_condition_in_past_3_months),times_had_chronic_condition_past_3=VALUES(times_had_chronic_condition_past_3),hospitalised_past_12_months=VALUES(hospitalised_past_12_months),
     hospitalised_for_respiratory_problem=VALUES(hospitalised_for_respiratory_problem),hospitalised_chronic_condition_past_12_months=VALUES(hospitalised_chronic_condition_past_12_months),smoke=VALUES(smoke),tobacco_products=VALUES(tobacco_products),influenza_vaccine=VALUES(influenza_vaccine),doses_of_pneumococcal_vaccine=VALUES(doses_of_pneumococcal_vaccine),pneumococcal_child_verification_card=VALUES(pneumococcal_child_verification_card),hib_vaccine=VALUES(hib_vaccine), doses_of_hib_vaccine=VALUES(doses_of_hib_vaccine),hib_child_vaccination_card=VALUES(hib_child_vaccination_card),
      voided=VALUES(voided);
    SELECT "Completed updating Sari data ", CONCAT("Time: ", NOW());
    END $$
...
select
    ...
    max(if(o.concept_id=1436,(case o.value_coded when 1065 then "YES" when 1066 THEN "NO" when 162570 THEN "Declined to answer" else "" end),null)) as partner_hiv_status,
...
CREATE PROCEDURE sp_scheduled_updates()
  BEGIN
    ...
    CALL sp_update_etl_patient_sari(last_update_time);
