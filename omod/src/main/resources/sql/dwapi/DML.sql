...
121764, 164933, 160080, 1823, 164940, 164934, 164935, 159615, 160288, 1855, 164947,162549,162877,160596,1109,1113,162309,1729,162737,159615,1120,163309,164936,1123,1124,1125,164937,1126,166607,159356,161011,165911,167161,165086,164946,165287,165287,164946,164941)
			...
			from encounter e
				...
				inner join obs o on e.encounter_id = o.encounter_id and o.voided =0
														and o.concept_id in(1282,159922,984,1418,1425,5088,5087,5085,5086,5242,5092,5089,5090,1343,21,163590,5245,1438,1439,160090,162089,1440,162107,5356,5497,856,1305,1147,159427,164848,161557,1436,1109,
														                    5576,128256,1875,159734,161438,161439,161440,161441,161442,161444,161443,162106,162101,162096,299,159918,32,119481,165099,120198,374,161074,1659,164934,163589,165040,166665,
														                    162747,1912,160481,163145,5096,159395,163784,1271,159853,165302,1592,1591,2031577,1592,161595,299,2031735,163783,163530,160080,5624,1427,5596,1438,167958,1000443,164359)
				inner join
				(
...
CREATE PROCEDURE sp_populate_dwapi_prep_monthly_refill()
  BEGIN
    ...
    from encounter e
           ...
           inner join obs o on o.encounter_id = e.encounter_id and o.concept_id in (1169,162189,164075,160582,160632,165144,167788,164425,166866,161641,1417,164515,164433,161555,164999,161011,5096,138643,165055,166535) and o.voided=0
    group by e.encounter_id;
...
CREATE PROCEDURE sp_populate_dwapi_prep_enrolment()
  BEGIN
    ...
    from encounter e
      ...
      inner join obs o on o.encounter_id = e.encounter_id and o.concept_id in (164932,160540,162724,161550,160534,160535,160555,159599,160533,1088162881,5629,160638,165038,160640,160642,160641,164930,160581,166866,166535) and o.voided=0
    group by e.encounter_id;
  ...
  BEGIN
    ...
    from encounter e
             ...
             inner join obs o on o.encounter_id = e.encounter_id and o.concept_id in (161558,165098,165200,165308,165099,1272,1472,5272,5596,1426,164933,5632,160653,374,
                                                                                      ...
                                                                                      165109,167788,165144,166866,159777,165055,165309,5096,165310,163042,134346,164075,160582,160632,1417,164515,164433,165353,165354,159623,165241,166535) and o.voided=0
    group by e.patient_id,visit_date;
...
BEGIN
    ...
    select
        ...
        max(if(vaccine='Tetanus Toxoid' and sequence=4, date_given, null)) as tetanus_taxoid_5,
...
DROP PROCEDURE IF EXISTS sp_populate_dwapi_etl_doctor_progress_note $$
CREATE PROCEDURE sp_populate_dwapi_etl_doctor_progress_note()
BEGIN
    SELECT "Processing Doctor's progress note data... ";
    insert into dwapi_etl.etl_doctor_progress_note(
        patient_id,
        uuid,
        provider,
        visit_id,
...
CALL sp_populate_dwapi_etl_doctor_progress_note();
