...
121764, 164933, 160080, 1823, 164940, 164934,159948, 164935,166970, 159615, 160288, 1855, 164947,162549,162877,160596,1109,1113,162309,1729,162737,159615,1120,163309,164936,1123,1124,1125,164937,1126,166607,159356,161011,165911)
where e.voided=0
      ...
      date_last_modified,
        ...
        max(if(o.concept_id=160325,o.value_coded,null)) as hpv_vaccinated,
			...
			from encounter e
				...
				left outer join obs o on o.encounter_id=e.encounter_id and o.voided=0
				and o.concept_id in (159368,1154,5219,160430,5089,5090,5085,5086,5088,5087,5242,5092,1343,163515,163300,1427,160325,162584)
			where e.voided=0
			group by e.patient_id, visit_date
		;
...
from encounter e
   inner join person p on p.person_id=e.patient_id and p.voided=0
   inner join (
              select encounter_type_id, uuid, name from encounter_type where uuid in('a0034eee-1940-4e35-847f-97537a35d05e','c6d09e05-1f25-4164-8860-9f32c5a02df0','c4a2be28-6673-4c36-b886-ea89b0a42116','706a8b12-c4ce-40e4-aec3-258b989bf6d3','a2010bf5-2db0-4bf4-819f-8a3cffbcb21b','d1059fb9-a079-4feb-a749-eedd709ae542')
              ) et on et.encounter_type_id=e.encounter_type
   inner join (select o.person_id,o1.encounter_id, o.obs_id,o.concept_id as obs_group,o1.concept_id as concept_id,o1.value_coded, o1.value_datetime,
                      o1.date_created,o1.voided from obs o join obs o1 on o.obs_id = o1.obs_group_id
                       and o1.concept_id in (5219,1284,159948,160643,159935,162760,160753,166937) and o1.voided=0
                       and o.concept_id in(160531,159392,121689)) o1 on o1.encounter_id = e.encounter_id
where e.voided=0
group by o1.obs_id;
