...
-- create table etl_covid-19_assessment
create table kenyaemr_datatools.covid-19_assessment as
  select
    uuid,
    provider,
    patient_id,
    ...
    166379 then 'Sinopharm' when 1067 then 'Unknown' when 5622 'Other(Specify)' end) as first_vaccine_type,
    (case second_vaccine_type when 166156 then 'Astrazeneca' when 166355 then 'Johnson and Johnson'
    when 166154 then 'Moderna' when 166155 then 'Pfizer' when 166157 then 'Sputnik' when
    166379 then 'Sinopharm' when 1067 then 'Unknown' when 5622 'Other(Specify)' end) as second_vaccine_type,
    first_dose,
    second_dose,
    first_dose_date,
    second_dose_date,
    ...
    (case final_vaccination_status when 166192 then 'Partially Vaccinated' when 5585 'Fully Vaccinated' end) as final_vaccination_status,
    ...
    when 166154 then 'Moderna' when 166155 then 'Pfizer' when 166157 then 'Sputnik' when
    166379 then 'Sinopharm' when 1067 then 'Unknown' when 5622 'Other(Specify)' end) as booster_vaccine_taken,
    date_taken_booster_vaccine,
    booster_dose,
    (case booster_dose_verified when 164134 then 'Yes' end) as booster_dose_verified,
    (case ever_tested_covid-19_positive when 703 then 'Yes' when 664 then 'No' when 1067 then 'Unknown' end ever_tested_covid-19_positive) as ,
    (case symptomatic_before_first_visit when 1068 then 'Yes' when 165912 then 'No' END) symptomatic_before_first_visit,
    ...
    (case covid-19_management_service_offered when 162738 then 'Supplemental oxygen' when 165932 then 'Ventilator'
    when 1067 then 'Unknown' when 1107 then 'None' end) as covid-19_management_service_offered,
    date_created,
    date_last_modified,
    voided
  from kenyaemr_etl.etl_covid-19_assessment;

ALTER TABLE kenyaemr_datatools.covid-19_assessment ADD FOREIGN KEY (patient_id) REFERENCES kenyaemr_datatools.patient_demographics(patient_id);
ALTER TABLE kenyaemr_datatools.covid-19_assessment ADD INDEX(visit_date);
SELECT "Successfully created covid-19_assessment table";
