...
-- ADD INDICES
    ...
    ALTER TABLE kenyaemr_datatools.patient_demographics
...
-- --------------------------- populate patient_hiv_enrollment table ---------------------------------------------
    ...
    select patient_id,
           ...
           (case entry_point
                ...
                when 5622 then "Other(eg STI)"
                else "" end)                                                                                    as entry_point,
           transfer_in_date,
           ...
           (case previous_regimen
                ...
                when 162559
                    then 'ABC/DDI/LPV/r' end)                                                                   as previous_regimen,
           ...
           IF(who_stage in (1204, 1220), "WHO Stage1", IF(who_stage in (1205, 1221), "WHO Stage2",
                                                          IF(who_stage in (1206, 1222), "WHO Stage3",
                                                             IF(who_stage in (1207, 1223), "WHO Stage4", "")))) as who_stage,
           name_of_treatment_supporter,
           (case relationship_of_treatment_supporter
                ...
                else "" end)                                                                                    as relationship_of_treatment_supporter,
           ...
           (case discontinuation_reason
                ...
                else "" end)                                                                                    as discontinuation_reason,
           voided
    from kenyaemr_etl.etl_hiv_enrollment;


    ALTER TABLE kenyaemr_datatools.hiv_enrollment
        ADD FOREIGN KEY (patient_id) REFERENCES kenyaemr_datatools.patient_demographics (patient_id);

    ALTER TABLE kenyaemr_datatools.hiv_enrollment
        ADD INDEX (visit_date);
    ALTER TABLE kenyaemr_datatools.hiv_enrollment
        ADD INDEX (arv_status);
    ALTER TABLE kenyaemr_datatools.hiv_enrollment
        ADD INDEX (date_confirmed_hiv_positive);
    ALTER TABLE kenyaemr_datatools.hiv_enrollment
        ADD INDEX (entry_point);
...
-- ----------------------------------- create table hiv_followup ----------------------------------------------
    ...
    select uuid,
           ...
           (case person_present
                when 978 then "Self (SF)"
                when 161642 then "Treatment supporter (TS)"
                when 5622 then "Other"
                else "" end)                                                                                    as person_present,
           weight,
           systolic_pressure,
           diastolic_pressure,
           height,
           ...
           (case population_type
                ...
                else "" end)                                                                                    as population_type,
           (case key_population_type
                ...
                else "" end)                                                                                    as key_population_type,
           IF(who_stage in (1204, 1220), "WHO Stage1", IF(who_stage in (1205, 1221), "WHO Stage2",
                                                          IF(who_stage in (1206, 1222), "WHO Stage3",
                                                             IF(who_stage in (1207, 1223), "WHO Stage4", "")))) as who_stage,
           ...
           screened_for_tb,
           (case spatum_smear_ordered
                when 307 then "Yes"
                when 1066 then "No"
                else "" end)                                                                                    as spatum_smear_ordered,
           (case chest_xray_ordered when 12 then "Yes" when 1066 then "No" else "" end)                         as chest_xray_ordered,
           (case genexpert_ordered
                when 162202 then "Yes"
                when 1066 then "No"
                else "" end)                                                                                    as genexpert_ordered,
           (case spatum_smear_result
                when 703 then "POSITIVE"
                when 664 then "NEGATIVE"
                else "" end)                                                                                    as spatum_smear_result,
           (case chest_xray_result
                when 1115 then "NORMAL"
                when 152526 then "ABNORMAL"
                else "" end)                                                                                    as chest_xray_result,
           (case genexpert_result
                when 664 then "NEGATIVE"
                when 162203 then "Mycobacterium tuberculosis detected with rifampin resistance"
                when 162204 then "Mycobacterium tuberculosis detected without rifampin resistance"
                ...
                else "" end)                                                                                    as genexpert_result,
           (case referral when 1065 then "Yes" when 1066 then "No" else "" end)                                 as referral,
           (case clinical_tb_diagnosis
                when 703 then "POSITIVE"
                when 664 then "NEGATIVE"
                else "" end)                                                                                    as clinical_tb_diagnosis,
           ...
           (case has_chronic_illnesses_cormobidities
                when 1065 then "Yes"
                when 1066 then "No"
                else "" end)                                                                                    as has_chronic_illnesses_cormobidities,
           (case has_adverse_drug_reaction
                when 1 then "Yes"
                when 0 then "No"
                else "" end)                                                                                    as has_adverse_drug_reaction,
           ...
           (case pregnancy_outcome
                when 126127 then "Spontaneous abortion"
                when 125872 then "STILLBIRTH"
                when 1395 then "Term birth of newborn"
                when 129218 then "Preterm Delivery (Maternal Condition)"
                when 159896 then "Therapeutic abortion procedure"
                when 151849 then "Liveborn, Unspecified Whether Single, Twin, or Multiple"
                when 1067 then "Unknown"
                else "" end)                                                                                    as pregnancy_outcome,
           anc_number,
           expected_delivery_date,
           (case ever_had_menses
                when 1065 then "Yes"
                when 1066 then "No"
                when 1175
                    then "N/A" end)                                                                             as ever_had_menses,
           ...
           (case family_planning_status
                ...
                else "" end)                                                                                    as family_planning_status,
           (case family_planning_method
                ...
                else "" end)                                                                                    as family_planning_method,
           (case reason_not_using_family_planning
                when 160572 then "Thinks can't get pregnant"
                when 160573 then "Not sexually active now"
                when 5622 then "Other"
                else "" end)                                                                                    as reason_not_using_family_planning,
           (case tb_status
           ...
           (case system_examination
                when 1115 then 'Normal'
                when 1116
                    then 'Abnormal' end)                                                                        as system_examination,
           (case skin_findings
                ...
                when 5244
                    then 'Oral sores' end)                                                                      as skin_findings,
           (case eyes_findings
                ...
                when 139100
                    then 'Itchy eyes' end)                                                                      as eyes_findings,
           (case ent_findings
                ...
                when 111525
                    then 'Vertigo' end)                                                                         as ent_findings,
           (case chest_findings
           ...
           (case abdomen_findings
                ...
                when 5105
                    then 'Abdominal tenderness' end)                                                            as abdomen_findings,
           (case cns_findings
                ...
                when 136282
                    then 'Kernicterus' end)                                                                     as cns_findings,
           (case genitourinary_findings
                ...
                when 124087
                    then 'Ulceration' end)                                                                      as genitourinary_findings,
           (case prophylaxis_given
                when 105281 then 'Cotrimoxazole'
                when 74250 then 'Dapsone'
                when 1107
                    then 'None' end)                                                                            as prophylaxis_given,
           (case ctx_adherence
                when 159405 then "Good"
                when 163794 then "Inadequate"
                when 159407 then "Poor"
                else "" end)                                                                                    as ctx_adherence,
           (case ctx_dispensed
                when 1065 then "Yes"
                when 1066 then "No"
                when 1175 then "Not applicable"
                else "" end)                                                                                    as ctx_dispensed,
           (case dapsone_adherence
                when 159405 then "Good"
                when 163794 then "Inadequate"
                when 159407 then "Poor"
                else "" end)                                                                                    as dapsone_adherence,
           (case dapsone_dispensed
                when 1065 then "Yes"
                when 1066 then "No"
                when 1175 then "Not applicable"
                else "" end)                                                                                    as dapsone_dispensed,
           (case inh_dispensed
                when 1065 then "Yes"
                when 1066 then "No"
                when 1175 then "Not applicable"
                else "" end)                                                                                    as inh_dispensed,
           (case arv_adherence
                when 159405 then "Good"
                when 163794 then "Inadequate"
                when 159407 then "Poor"
                else "" end)                                                                                    as arv_adherence,
           (case poor_arv_adherence_reason
                when 102 then "Toxicity, drug"
                when 121725 then "Alcohol abuse"
                when 119537 then "Depression"
                when 5622 then "Other"
                when 1754 then "Medications unavailable"
                when 1778 then "TREATMENT OR PROCEDURE NOT CARRIED OUT DUE TO FEAR OF SIDE EFFECTS"
                when 819 then "Cannot afford treatment"
                when 160583 then "Shares medications with others"
                when 160584 then "Lost or ran out of medication"
                when 160585 then "Felt too ill to take medication"
                when 160586 then "Felt better and stopped taking medication"
                when 160587 then "Forgot to take medication"
                when 160588 then "Pill burden"
                when 160589 then "Concerned about privacy/stigma"
                when 820 then "TRANSPORT PROBLEMS"
                else "" end)                                                                                    as poor_arv_adherence_reason,
           ...
           (case pwp_disclosure
                ...
                when 1067 then "Unknown"
                when 1175 then "N/A"
                else "" end)                                                                                    as pwp_disclosure,
           (case pwp_pead_disclosure
                when 1066 then "No disclosure"
                when 162979 then "Partial disclosure"
                when 166982 then "Full disclosure"
                else "" end)                                                                                    as pwp_pead_disclosure,
           (case pwp_partner_tested
                when 1065 then "Yes"
                when 1066 then "No"
                when 1067 then "Unknown"
                when 1175 then "N/A"
                else "" end)                                                                                    as pwp_partner_tested,
           (case condom_provided
                when 1065 then "Yes"
                when 1066 then "No"
                when 1067 then "Unknown"
                when 1175 then "N/A"
                else "" end)                                                                                    as condom_provided,
           (case substance_abuse_screening
                when 1065 then "Yes"
                when 1066 then "No"
                when 1067 then "Unknown"
                else "" end)                                                                                    as substance_abuse_screening,
           (case screened_for_sti
                when 703 then "POSITIVE"
                when 664 then "NEGATIVE"
                when 1118 then "Not Done"
                when 1175 then "N/A"
                else "" end)                                                                                    as screened_for_sti,
           (case cacx_screening
                when 703 then "POSITIVE"
                when 664 then "NEGATIVE"
                when 1118 then "Not Done"
                when 1175 then "N/A"
                else "" end)                                                                                    as cacx_screening,
           (case sti_partner_notification
                when 1065 then "Yes"
                when 1066 then "No"
                else "" end)                                                                                    as sti_partner_notification,
           (case at_risk_population
                when 105 then "People who inject drugs"
                when 160578 then "Men who have sex with men"
                when 160579 then "Female sex Worker"
                else "" end)                                                                                    as at_risk_population,
           (case system_review_finding
                when 1115 then "NORMAL"
                when 1116 then "ABNORMAL"
                else "" end)                                                                                    as system_review_finding,
           next_appointment_date,
           refill_date,
           (case appointment_consent
                when 1065 then "Yes"
                when 1066 then "No"
                else "" end)                                                                                    as appointment_consent,
           (case next_appointment_reason
                ...
                when 5622 then "Other"
                else "" end)                                                                                    as next_appointment_reason,
           (case stability
                when 1 then "Yes"
                when 2 then "No"
                when 0 then "No"
                when 1175 then "Not applicable"
                else "" end)                                                                                    as stability,
           (case differentiated_care
                ...
                else "" end)                                                                                    as differentiated_care,
           (case insurance_type
                when 1917 then "NHIF"
                when 1107 then "None"
                when 5622 then "Other"
           ...
           (case insurance_status
                when 161636 then "Active"
                when 1118 then "Inactive"
                else "" end)                                                                                    as insurance_status
    from kenyaemr_etl.etl_patient_hiv_followup;

    ALTER TABLE kenyaemr_datatools.hiv_followup
        ADD FOREIGN KEY (patient_id) REFERENCES kenyaemr_datatools.patient_demographics (patient_id);

    ALTER TABLE kenyaemr_datatools.hiv_followup
        ADD INDEX (visit_date);
    ALTER TABLE kenyaemr_datatools.hiv_followup
        ADD INDEX (pregnancy_status);
    ALTER TABLE kenyaemr_datatools.hiv_followup
        ADD INDEX (breastfeeding);
    ALTER TABLE kenyaemr_datatools.hiv_followup
        ADD INDEX (family_planning_status);
    ALTER TABLE kenyaemr_datatools.hiv_followup
        ADD INDEX (tb_status);
    ALTER TABLE kenyaemr_datatools.hiv_followup
        ADD INDEX (ctx_dispensed);
    ALTER TABLE kenyaemr_datatools.hiv_followup
        ADD INDEX (population_type);
    ALTER TABLE kenyaemr_datatools.hiv_followup
        ADD INDEX (on_anti_tb_drugs);
    ALTER TABLE kenyaemr_datatools.hiv_followup
        ADD INDEX (stability);
    ALTER TABLE kenyaemr_datatools.hiv_followup
        ADD INDEX (differentiated_care);
...
-- -------------------------------- create table laboratory_extract ------------------------------------------
    ...
    select uuid,
           ...
           (case lab_test
                ...
                when 45 then "Urine Pregnancy Test"
           ...
           if(lab_test = 299, (case test_result
              ...
              if(lab_test = 1030, (case test_result
                                       ...
                                       when 703 then "POSITIVE"
                                       when 1304 then "POOR SAMPLE QUALITY" end),
                 if(lab_test = 163722, (case test_result
                                            when 1138 then "INDETERMINATE"
                                            when 664 then "NEGATIVE"
                                            when 703 then "POSITIVE"
                                            when 1304 then "POOR SAMPLE QUALITY" end),
                    if(lab_test = 302, (case test_result
                       ...
                       if(lab_test = 32, (case test_result
                          ...
                          if(lab_test = 1305, (case test_result
                                                   ...
                                                   when 1302 then "LDL"
                                                   when 1304 then "POOR SAMPLE QUALITY" end),
                             if(lab_test = 167718, (case test_result
                                                        when 1254 then "CD4 COUNT GREATER THAN 200"
                                                        when 167717 then "CD4 count less than or equal to 200" end),
                                if(lab_test = 1029,
                                         ...
                                         if(lab_test = 307, (case test_result
                                            ...
                                            if(lab_test = 162202, (case test_result
                                               ...
                                               if(lab_test = 1619, (case test_result
                                                                        ...
                                                                        when 664 then "Negative"
                                                                        when 1067 then "Unknown" end),
                                                  if(lab_test = 45, (case test_result
                                                                         when 703 then "Positive"
                                                                         when 664 then "Negative"
                                                                         when 1138 then "Indeterminate"
                                                                         when 1304 then "Poor Quality Sample" end),
                                                     if(lab_test = 167452, (case test_result
                                                                                when 703 then "Positive"
                                                                                when 664 then "Negative"
                                                                                when 1067 then "Unknown" end),
                                                        if(lab_test = 167459,
                                                           (case test_result when 163747 then "Absent" when 163748 then "Present" end),
                                                           test_result)))))))))))))))) AS test_result,
           date_created,
           created_by
    from kenyaemr_etl.etl_laboratory_extract;

    ALTER TABLE kenyaemr_datatools.laboratory_extract
        ADD FOREIGN KEY (patient_id) REFERENCES kenyaemr_datatools.patient_demographics (patient_id);
...
-- create table pharmacy_extract
    ...
    ALTER TABLE kenyaemr_datatools.pharmacy_extract
        ADD FOREIGN KEY (patient_id) REFERENCES kenyaemr_datatools.patient_demographics (patient_id);
...
-- create table patient_program_discontinuation
    ...
    select patient_id,
           ...
           (case discontinuation_reason
                when 159492 then "Transferred Out"
                when 160034 then "Died"
                when 160432 then "Died"
                when 5240 then "Lost to Follow"
                when 819 then "Cannot afford Treatment"
                when 5622 then "Other"
                when 1067 then "Unknown"
                when 164349 then "Stopped Treatment"
                else "" end) as discontinuation_reason,
           date_died,
           transfer_facility,
           transfer_date,
    ...
    ALTER TABLE kenyaemr_datatools.patient_program_discontinuation
        ADD FOREIGN KEY (patient_id) REFERENCES kenyaemr_datatools.patient_demographics (patient_id);

    ALTER TABLE kenyaemr_datatools.patient_program_discontinuation
        ADD INDEX (visit_date);
    ALTER TABLE kenyaemr_datatools.patient_program_discontinuation
        ADD INDEX (discontinuation_reason);
    ...
    select patient_id,
           ...
           (case urinary_albumin
                when 664 then "Negative"
                when 1874 then "Trace - 15"
                when 1362 then "One Plus(+) - 30"
                when 1363 then "Two Plus(++) - 100"
                when 1364 then "Three Plus(+++) - 300"
                when 1365 then "Four Plus(++++) - 1000"
                else "" end)                                                                 as urinary_albumin,
           (case glucose_measurement
                when 1115 then "Normal"
                when 1874 then "Trace"
                when 1362 then "One Plus(+)"
                when 1363 then "Two Plus(++)"
                when 1364 then "Three Plus(+++)"
                when 1365 then "Four Plus(++++)"
                else "" end)                                                                 as glucose_measurement,
           urine_ph,
           urine_gravity,
           (case urine_nitrite_test
                ...
                when 1362 then "One Plus(+)"
                when 1363 then "Two Plus(++)"
                else "" end)                                                                 as urine_nitrite_test,
           (case urine_leukocyte_esterace_test
                ...
                when 1363 then "Two Plus(++)"
                when 1364 then "Three Plus(+++)"
                else "" end)                                                                 as urine_leukocyte_esterace_test,
           (case urinary_ketone
                when 664 then "NEGATIVE"
                when 1874 then "Trace - 5"
                when 1362 then "One Plus(+) - 15"
                when 1363 then "Two Plus(++) - 50"
                when 1364 then "Three Plus(+++) - 150"
                else "" end)                                                                 as urinary_ketone,
           (case urine_bile_salt_test
                when 1115 then "Normal"
                when 1874 then "Trace - 1"
                when 1362 then "One Plus(+) - 4"
                when 1363 then "Two Plus(++) - 8"
                when 1364 then "Three Plus(+++) - 12"
                else "" end)                                                                 as urine_bile_salt_test,
           (case urine_bile_pigment_test
                when 664 then "NEGATIVE"
                when 1362 then "One Plus(+)"
                when 1363 then "Two Plus(++)"
                when 1364 then "Three Plus(+++)"
                else "" end)                                                                 as urine_bile_pigment_test,
           (case urine_colour
                when 162099 then "Colourless"
                when 127778 then "Red color"
                when 162097 then "Light yellow colour"
                when 162105 then "Yellow-green colour"
                when 162098 then "Dark yellow colour"
                when 162100 then "Brown color"
                else "" end)                                                                 as urine_colour,
           (case urine_turbidity
                when 162102 then "Urine appears clear"
                when 162103 then "Cloudy urine"
                when 162104 then "Urine appears turbid"
                else "" end)                                                                 as urine_turbidity,
           (case urine_dipstick_for_blood
                ...
                when 1363 then "Two Plus(++)"
                when 1364 then "Three Plus(+++)"
                else "" end)                                                                 as urine_dipstick_for_blood,
           (case discontinuation_reason
                when 159492 then "Transferred out"
                when 1067 then "Unknown"
                when 160034 then "Died"
    ...
    ALTER TABLE kenyaemr_datatools.mch_enrollment
        ADD FOREIGN KEY (patient_id) REFERENCES kenyaemr_datatools.patient_demographics (patient_id);
    ALTER TABLE kenyaemr_datatools.mch_enrollment
        ADD INDEX (visit_date);
    ...
    select patient_id,
           ...
           maturity,
           fundal_height,
           (case fetal_presentation
                when 139814 then "Frank Breech Presentation"
                when 160091 then "vertex presentation"
                when 144433 then "Compound Presentation"
                when 115808 then "Mentum Presentation of Fetus"
                when 118388 then "Face or Brow Presentation of Foetus"
                when 129192 then "Presentation of Cord"
                when 112259 then "Transverse or Oblique Fetal Presentation"
                when 164148 then "Occiput Anterior Position"
                when 164149 then "Brow Presentation"
                when 164150 then "Face Presentation"
                when 156352 then "footling breech presentation"
                else "" end)                                                              as fetal_presentation,
           (case lie
                when 132623 then "Oblique lie"
                when 162088 then "Longitudinal lie"
                when 124261 then "Transverse lie"
                else "" end)                                                              as lie,
           fetal_heart_rate,
           (case fetal_movement
                when 162090 then "Increased fetal movements"
                when 113377 then "Decreased fetal movements"
                when 1452 then "No fetal movements"
                when 162108 then "Fetal movements present"
                else "" end)                                                              as fetal_movement,
           (case who_stage
                when 1204 then "WHO Stage1"
                when 1205 then "WHO Stage2"
                when 1206 then "WHO Stage3"
                when 1207 then "WHO Stage4"
                else "" end)                                                              as who_stage,
           cd4,
           ...
           (case arv_status
                when 1148 then "ARV Prophylaxis"
                when 1149 then "HAART"
                when 1175 then "NA"
                else "" end)                                                              as arv_status,
           final_test_result,
           ...
           (case baby_azt_dispensed
                ...
                when 1066 then "No"
                when 1175 then "N/A"
                else "" end)                                                              as baby_azt_dispensed,
           (case baby_nvp_dispensed
                when 80586 then "Yes"
                when 1066 then "No"
                when 1175 then "N/A"
                else "" end)                                                              as baby_nvp_dispensed,
           deworming_done_anc,
           IPT_dose_given_anc,
           TTT,
           IPT_malaria,
           ...
           (case syphilis_test_status
                when 1229 then "Non Reactive"
                when 1228 then "Reactive"
                when 1402 then "Not Screened"
                when 1304 then "Poor Sample quality"
                else "" end)                                                              as syphilis_test_status,
           (case syphilis_treated_status
                when 1065 then "Yes"
                when 1066 then "No"
                else "" end)                                                              as syphilis_treated_status,
           (case bs_mps
                when 664 then "Negative"
                when 703 then "Positive"
                when 1138 then "Indeterminate"
           ...
           (case diabetes_test
                ...
                else "" end)                                                              as diabetes_test,
           (case intermittent_presumptive_treatment_given
                when 1065 then "Yes"
                when 1066 then "No"
                when 1175
                    then "Not Applicable" end)                                            as intermittent_presumptive_treatment_given,
           (case intermittent_presumptive_treatment_dose
                ...
                when 0
                    then "No" end)                                                        as intermittent_presumptive_treatment_dose,
           ...
           (case fgm_complications
                when 122949 then "Scarring"
                when 136308 then "Keloids"
                when 141615 then "dyspaneuria"
                when 111633 then "UTI"
                else "" end)                                                              as fgm_complications,
           (case fp_method_postpartum
                when 5275 then "IUD"
                when 159589 then "Implants"
                when 1472 then "BTL"
                else "" end)                                                              as fp_method_postpartum,
           (case anc_exercises
                when 1065 then "Yes"
                when 1066 then "No"
                when 1067 then "Unknown"
                else "" end)                                                              as anc_exercises,
           (case tb_screening
                when 1660 then "No TB signs"
                ...
                when 1662 then "TB Confirmed"
                when 160737 then "Not done"
                when 1111 then "On TB Treatment"
           ...
           (case cacx_screening_method
                ...
                when 5622 then "Other"
                else "" end)                                                              as cacx_screening_method,
           (case hepatitis_b_screening
                when 703 then "Positive"
                when 664 then "Negative"
                when 160737 then "Not Done"
                else "" end)                                                              as hepatitis_b_screening,
           (case hepatitis_b_treatment
                when 1065 then "Yes"
                when 1066 then "No"
                else "" end)                                                              as hepatitis_b_treatment,
    ...
    ALTER TABLE kenyaemr_datatools.mch_antenatal_visit
        ADD FOREIGN KEY (patient_id) REFERENCES kenyaemr_datatools.patient_demographics (patient_id);
    ALTER TABLE kenyaemr_datatools.mch_antenatal_visit
        ADD INDEX (visit_date);
...
-- create table mch_delivery table
    ...
    select patient_id,
           ...
           (case vaginal_examination
                when 132681 then "Normal"
                when 5577 then "Episiotomy"
                when 159264 then "Vaginal Tear"
                when 118935 then "FGM"
                when 139505 then "Vaginal wart"
                else "" end)                                                              as vaginal_examination,
           (case uterotonic_given
                when 81369 then "Oxytocin"
                when 104590 then "Carbetocin"
                when 1107 then "None"
                else "" end)                                                              as uterotonic_given,
           (case chlohexidine_applied_on_code_stump
                when 1065 then "Yes"
                when 1066 then "No"
                else "" end)                                                              as chlohexidine_applied_on_code_stump,
           (case vitamin_K_given when 1065 then "Yes" when 1066 then "No" else "" end)    as vitamin_K_given,
           (case kangaroo_mother_care_given
                when 1065 then "Yes"
                when 1066 then "No"
                when 1175 then "N/A"
                else "" end)                                                              as kangaroo_mother_care_given,
           (case testing_done_in_the_maternity_hiv_status
                when 703 then "Positive"
                when 664 then "Negative"
                when 1067 then "Unknown"
                when 164817 then "Known Positive"
                else "" end)                                                              as testing_done_in_the_maternity_hiv_status,
           (case infant_provided_with_arv_prophylaxis
                when 1065 then "Yes"
                when 1066 then "No"
                when 1067 then "N/A"
                else "" end)                                                              as infant_provided_with_arv_prophylaxis,
           (case mother_on_haart_during_anc
                when 1065 then "Yes"
                when 1066 then "No"
                when 1067 then "N/A"
                else "" end)                                                              as mother_on_haart_during_anc,
           (case mother_started_haart_at_maternity
                when 1065 then "Yes"
                when 1066 then "No"
                when 1067 then "N/A"
                else "" end)                                                              as mother_started_haart_at_maternity,
           (case vdrl_rpr_results
                when 703 then "Positive"
                when 664 then "Negative"
                when 1118 then "Not Done"
                else "" end)                                                              as vdrl_rpr_results,
           ...
           (case mode_of_delivery
                ...
                when 118159 then "Forceps or Vacuum Extractor Delivery"
                when 159739 then "emergency caesarean section"
                when 159260 then "vacuum extractor delivery"
                when 5622 then "Other"
                when 1067 then "Unknown"
                else "" end)                                                              as mode_of_delivery,
           date_of_delivery,
           blood_loss,
           (case condition_of_mother
                when 160429 then "Alive"
                when 134612 then "Dead"
                else "" end)                                                              as condition_of_mother,
           (case delivery_outcome
                when 159913 then 'Single'
                when 159914 then 'Twins'
                when 159915
                    then 'Triplets' end)                                                  as delivery_outcome,
           apgar_score_1min,
           apgar_score_5min,
           apgar_score_10min,
           (case resuscitation_done
                ...
                when 1066 then "No"
                when 1067 then "Unknown"
                else "" end)                                                              as resuscitation_done,
           (case place_of_delivery
                when 1536 then "HOME"
                when 1588 then "HEALTH CLINIC/POST"
                when 1589 then "HOSPITAL"
                when 1601 then "EN ROUTE TO HEALTH FACILITY"
                when 159670 then "sub-district hospital"
                when 159671 then "Provincial hospital"
                when 159662 then "district hospital"
                when 159372 then "Primary Care Clinic"
                when 5622 then "Other"
                when 1067 then "Unknown"
                else "" end)                                                              as place_of_delivery,
           delivery_assistant,
           (case counseling_on_infant_feeding
                when 161651 then "Counseling about infant feeding practices"
                else "" end)                                                              as counseling_on_infant_feeding,
           (case counseling_on_exclusive_breastfeeding
                when 161096 then "Counseling for exclusive breastfeeding"
                else "" end)                                                              as counseling_on_exclusive_breastfeeding,
           (case counseling_on_infant_feeding_for_hiv_infected
                when 162091 then "Counseling for infant feeding practices to prevent HIV"
                else "" end)                                                              as counseling_on_infant_feeding_for_hiv_infected,
           (case mother_decision
                when 1173 then "EXPRESSED BREASTMILK"
                when 1152 then "WEANED"
                when 5254 then "Infant formula"
                when 1150 then "BREASTFED PREDOMINATELY"
                when 6046 then "Mixed feeding"
                when 5526 then "BREASTFED EXCLUSIVELY"
                when 968 then "COW MILK"
                when 1595 then "REPLACEMENT FEEDING"
                else "" end)                                                              as mother_decision,
           (case placenta_complete
                when 703 then "Yes"
                when 664 then "No"
                when 1501 then "Baby born before arrival"
                else "" end)                                                              as placenta_complete,
           (case maternal_death_audited
                when 1065 then "Yes"
                when 1066 then "No"
                else "" end)                                                              as maternal_death_audited,
           (case cadre
                when 1574 then "CLINICAL OFFICER/DOCTOR"
                when 1578 then "Midwife"
                when 1577 then "NURSE"
                when 1575 then "TRADITIONAL BIRTH ATTENDANT"
                when 1555 then " COMMUNITY HEALTH CARE WORKER"
                when 5622 then "Other"
                else "" end)                                                              as cadre,
           (case delivery_complications
                when 1065 then "Yes"
                when 1066 then "No"
                else "" end)                                                              as delivery_complications,
           (case coded_delivery_complications
                ...
                when 1067 then "Unknown"
                else "" end)                                                              as coded_delivery_complications,
           other_delivery_complications,
           duration_of_labor,
           (case baby_sex
                when 1534 then "Male Gender"
                when 1535 then "Female gender"
                else "" end)                                                              as baby_sex,
           (case baby_condition
                when 135436 then "Macerated Stillbirth"
                when 159916 then "Fresh stillbirth"
                when 151849 then "Live birth"
                when 125872 then "STILLBIRTH"
                when 126127 then "Spontaneous abortion"
                when 164815 then "Live birth, died before arrival at facility"
                when 164816 then "Live birth, died after arrival or delivery in facility"
                else "" end)                                                              as baby_condition,
           (case teo_given
                when 84893 then "TETRACYCLINE"
                when 1066 then "No"
                when 1175 then "Not applicable"
                else "" end)                                                              as teo_given,
           birth_weight,
           (case bf_within_one_hour when 1065 then "Yes" when 1066 then "No" else "" end) as bf_within_one_hour,
           (case birth_with_deformity
                when 155871 then "deformity"
                when 1066 then "No"
                when 1175 then "Not applicable"
                else "" end)                                                              as birth_with_deformity,
           final_test_result,
           patient_given_result,
           (case partner_hiv_tested when 1065 then "Yes" when 1066 then "No" else "" end) as partner_hiv_tested,
           (case partner_hiv_status
           ...
           (case prophylaxis_given
                when 105281 then "SULFAMETHOXAZOLE / TRIMETHOPRIM"
                when 74250 then "DAPSONE"
                when 1107 then "None"
                else "" end)                                                              as prophylaxis_given,
           (case baby_azt_dispensed
                when 160123 then "Zidovudine for PMTCT"
                when 1066 then "No"
                when 1175 then "Not Applicable"
                else "" end)                                                              as baby_azt_dispensed,
           (case baby_nvp_dispensed
    ...
    ALTER TABLE kenyaemr_datatools.mch_delivery
        ADD FOREIGN KEY (patient_id) REFERENCES kenyaemr_datatools.patient_demographics (patient_id);
    ALTER TABLE kenyaemr_datatools.mch_delivery
        ADD INDEX (visit_date);
...
-- create table mch_discharge table
    ...
    select patient_id,
           ...
           (case baby_status
                when 163016 then "Alive"
                when 160432 then "Dead"
                else "" end)                                                                 as baby_status,
           (case vitamin_A_dispensed
                when 1065 then "Yes"
                when 1066 then "No"
                when 1067 then "Unknown"
                else "" end)                                                                 as vitamin_A_dispensed,
    ...
    ALTER TABLE kenyaemr_datatools.mch_discharge
        ADD FOREIGN KEY (patient_id) REFERENCES kenyaemr_datatools.patient_demographics (patient_id);
    ALTER TABLE kenyaemr_datatools.mch_discharge
        ADD INDEX (visit_date);
    ...
    select patient_id,
           ...
           (case mode_of_delivery
                when 1170 then "SVD"
                when 1171 then "C-Section"
                when 1172 then "Breech delivery"
                when 118159 then "Assisted vaginal delivery"
                else "" end)                                                              as mode_of_delivery,
           (case place_of_delivery
           ...
           (case visit_timing_mother
                when 1721 then '0-48 Hours'
                when 1722 then '3 days - 6 weeks'
                when 1723
                    then 'More than 6 weeks' end)                                         as visit_timing_mother,
           (case visit_timing_baby
                when 167012 then '0-48 Hours'
                when 167013 then '3 days - 6 weeks'
                when 167015
                    then 'More than 6 weeks' end)                                         as visit_timing_baby,
           (case delivery_outcome
                when 159913 then 'Single'
                when 159914 then 'Twins'
                when 159915
           ...
           (case general_condition
                ...
                when 162132 then "Poor"
                else "" end)                                                              as general_condition,
           (case breast
                when 1855 then "Good"
                when 162133 then "Fair"
                when 162132 then "Poor"
                else "" end)                                                              as breast, -- recheck
           (case cs_scar
           ...
           (case gravid_uterus
                ...
                when 162119 then "On exam, uterine fundus 38 weeks-term size"
                when 123427 then "Uterus Involuted"
                else "" end)                                                              as gravid_uterus,
           (case episiotomy
                when 159842 then "repaired, episiotomy wound"
                when 159843 then "healed, episiotomy wound"
                when 159841 then "gap, episiotomy wound"
                when 113919 then "Postoperative Wound Infection"
                else "" end)                                                              as episiotomy,
           (case lochia
                when 159845 then "lochia excessive"
                when 159846 then "lochia foul smelling"
                when 159721 then "Lochia type"
                else "" end)                                                              as lochia, -- recheck
           (case counselled_on_infant_feeding
                when 1065 then 'Yes'
                when 1066
                    then 'No' end)                                                        as counselled_on_infant_feeding,
           (case pallor
                when 1065 then "Yes"
                when 1066 then "No"
                when 1175 then "Not applicable"
                else "" end)                                                              as pallor,
           (case pallor_severity
                when 1498 then 'Mild'
           ...
           (case mother_hiv_status
                when 1067 then "Unknown"
                when 664 then "NEGATIVE"
                when 703 then "POSITIVE"
                else "" end)                                                              as mother_hiv_status,
           (case condition_of_baby
                when 1855 then "In good health"
                when 162132 then "Patient condition poor"
                when 1067 then "Unknown"
                when 162133 then "Patient condition fair/satisfactory"
                else "" end)                                                              as condition_of_baby,
           (case baby_feeding_method
                when 5526 then "BREASTFED EXCLUSIVELY"
                when 1595 then "REPLACEMENT FEEDING"
                when 6046 then "Mixed feeding"
                when 159418 then "Not at all sure"
                else "" end)                                                              as baby_feeding_method,
           (case umblical_cord
                ...
                when 159418 then "Not at all sure"
                else "" end)                                                              as umblical_cord,
           (case baby_immunization_started
                when 1065 then "Yes"
                when 1066 then "No"
                when 1067 then "Unknown"
                else "" end)                                                              as baby_immunization_started,
           (case family_planning_counseling
                when 1065 then "Yes"
                when 1066 then "No"
                when 1067 then "Unknown"
                else "" end)                                                              as family_planning_counseling,
           other_maternal_complications,
           (case uterus_examination
                when 163750 then "Contracted"
                when 148220 then "Not contracted"
                when 5622 then "Other"
                else "" end)                                                              as uterus_examination,
           ...
           final_test_result,
           (case syphilis_results
                when 1229 then "Positive"
                when 1228
                    then "Negative" end)                                                  as syphilis_results,
           patient_given_result,
           ...
           (case pnc_hiv_test_timing_mother
                when 162080 then "Less than 6 weeks"
                when 162081 then "Greater 6 weeks"
                when 1118
                    then "Not Done" end)                                                  as pnc_hiv_test_timing_mother,
           (case mother_haart_given
                when 1065 then 'Yes'
                when 1066 then 'No'
                when 1175 then 'N/A'
                when 164142 then 'Revisit'
                else '' end)                                                              as mother_haart_given,
           (case prophylaxis_given
                when 105281 then "Cotrimoxazole"
                when 74250 then "Dapsone"
                when 1107 then "None"
                else "" end)                                                              as prophylaxis_given,
           (case infant_prophylaxis_timing
                when 1065 then 'Less than 6 weeks'
                when 1066
                    then 'Greater 6 weeks' end)                                           as infant_prophylaxis_timing,
           (case baby_azt_dispensed
                when 160123 then "Yes"
                when 1066 then "No"
                when 1175 then "N/A"
           ...
           (case pnc_exercises
                ...
                when 1066 then "No"
                when 1067 then "Unknown"
                else "" end)                                                              as pnc_exercises,
           (case maternal_condition
                when 130 then "Puerperal sepsis"
                when 114244 then "Perineal Laceration"
                when 1855 then "In good health"
                when 134612 then "Maternal Death"
                when 160429 then "Alive"
                when 162132 then "Patient condition poor"
                when 162133 then "Patient condition fair/satisfactory"
                else "" end)                                                              as maternal_condition,
           (case iron_supplementation
                when 1065 then "Yes"
                when 1066 then "No"
                else "" end)                                                              as iron_supplementation,
           (case fistula_screening
                when 1107 then "None"
                when 49 then "Vesicovaginal Fistula"
                when 127847 then "Rectovaginal fistula"
                when 1118 then "Not done"
                else "" end)                                                              as fistula_screening,
           (case cacx_screening
                when 703 then "POSITIVE"
                when 664 then "NEGATIVE"
                when 159393 then "Presumed"
    ...
    ALTER TABLE kenyaemr_datatools.mch_postnatal_visit
        ADD FOREIGN KEY (patient_id) REFERENCES kenyaemr_datatools.patient_demographics (patient_id);
    ALTER TABLE kenyaemr_datatools.mch_postnatal_visit
        ADD INDEX (visit_date);
    ...
    select serial_no,
           ...
           (case birth_type
                ...
                when 113440
                    then 'Quintuplets' end)                                        as birth_type,
           date_first_seen,
           birth_notification_number,
           birth_certificate_number,
           (case need_for_special_care
                when 161628 then "Yes"
                when 1066 then "No"
                else "" end)                                                       as need_for_special_care,
           (case reason_for_special_care
                ...
                when 123174 then "History/signs of child abuse/neglect"
                else "" end)                                                       as reason_for_special_care,
           ...
           (case mother_breastfeeding
                ...
                when 1066 then "No"
                when 1067 then "Unknown"
                else "" end)                                                       as mother_breastfeeding,
           (case TB_contact_history_in_household
                when 1065 then "Yes"
                when 1066 then "No"
                else "" end)                                                       as TB_contact_history_in_household,
           (case mother_alive when 1 then "Yes" when 0 then "No" else "" end)      as mother_alive,
           (case mother_on_pmtct_drugs
                when 1065 then "Yes"
                when 1066 then "No"
                else "" end)                                                       as mother_on_pmtct_drugs,
           (case mother_on_drug
                when 80586 then "Sd NVP Only"
                when 1652 then "AZT+NVP+3TC"
                when 1149 then "HAART"
                when 1107 then "None"
                else "" end)                                                       as mother_on_drug,
           (case mother_on_art_at_infant_enrollment
                when 1065 then "Yes"
                when 1066 then "No"
                else "" end)                                                       as mother_on_art_at_infant_enrollment,
           (case mother_drug_regimen
                ...
                else "" end)                                                       as mother_drug_regimen,
           (case infant_prophylaxis
                when 80586 then "Sd NVP Only"
                when 1652 then "AZT/NVP"
                when 162326 then "NVP for 6 weeks(Mother on HAART)"
                when 160123 then "AZT Liquid BD for 6 weeks"
                when 78643 then "3TC Liquid BD"
                when 1149 then "none"
                when 1107 then "Other"
                else "" end)                                                       as infant_prophylaxis,
           parent_ccc_number,
           (case mode_of_delivery
                when 1170 then "SVD"
                when 1171 then "C-Section"
           ...
           exit_date,
           (case exit_reason
                when 1403 then "HIV Neg age greater 18 months"
                when 138571 then "Confirmed HIV Positive"
                when 5240 then "Lost"
                when 160432 then "Dead"
    ...
    ALTER TABLE kenyaemr_datatools.hei_enrollment
        ADD FOREIGN KEY (patient_id) REFERENCES kenyaemr_datatools.patient_demographics (patient_id);
    ALTER TABLE kenyaemr_datatools.hei_enrollment
        ADD INDEX (visit_date);
    ...
    select patient_id,
           ...
           (case primary_caregiver
                ...
                when 973 then "Guardian"
                when 972 then "Guardian"
                when 160639 then "Guardian"
                when 5622 then "Guardian"
                else "" end)                                                                                 as primary_caregiver,
           (case revisit_this_year when 1065 then "Yes" when 1066 then "No" end)                             as revisit_this_year,
           (case height_length
                when 1115 then "Normal"
                when 164085 then "Stunted"
                when 164086
                    then "Severe Stunded" end)                                                               as height_length,
           ...
           (case tb_assessment_outcome
                ...
                when 1679 then "INH"
                when 160737 then "TB Screening Not Done"
                else "" end)                                                                                 as tb_assessment_outcome,
           (case social_smile_milestone when 162056 then "Social Smile" else "" end)                         as social_smile_milestone,
           (case head_control_milestone
                when 162057 then "Head Holding/Control"
                else "" end)                                                                                 as head_control_milestone,
           (case response_to_sound_milestone
                when 162058 then "Turns towards the origin of sound"
                else "" end)                                                                                 as response_to_sound_milestone,
           (case hand_extension_milestone
                when 162059 then "Extends hand to grasp a toy"
                else "" end)                                                                                 as hand_extension_milestone,
           ...
           (case review_of_systems_developmental
                when 1115 then "Normal(N)"
                when 6022 then "Delayed(D)"
                when 6025 then "Regressed(R)"
                else "" end)                                                                                 as review_of_systems_developmental,
           (case weight_category
                ...
                when 1115 then "Normal(N)"
                else "" end)                                                                                 as weight_category,
           (case followup_type
                when 132636 then 'Marasmus'
                when 116474 then 'Kwashiorkor'
                when 115122 then 'Mulnutrition'
                when 5622 then 'Other'
                else "" end)                                                                                 as followup_type,
           dna_pcr_sample_date,
           (case dna_pcr_contextual_status
                when 162081 then "Repeat"
                when 162083 then "Final test (end of pediatric window)"
                when 162082 then "Confirmation"
                when 162080 then "Initial"
                else "" end)                                                                                 as dna_pcr_contextual_status,
           (case dna_pcr_result
                when 1138 then "INDETERMINATE"
                when 664 then "NEGATIVE"
                when 703 then "POSITIVE"
                when 1304 then "POOR SAMPLE QUALITY"
                else "" end)                                                                                 as dna_pcr_result,
           ...
           (case first_antibody_result
                when 664 then "NEGATIVE"
                when 703 then "POSITIVE"
                when 1304 then "POOR SAMPLE QUALITY"
                else "" end)                                                                                 as first_antibody_result,
           (case final_antibody_result
                when 664 then "NEGATIVE"
                when 703 then "POSITIVE"
                when 1304 then "POOR SAMPLE QUALITY"
                else "" end)                                                                                 as final_antibody_result,
           (case tetracycline_ointment_given
                when 1065 then "Yes"
                when 1066 then "No"
                else "" end)                                                                                 as tetracycline_ointment_given,
           (case pupil_examination
                when 162065 then "Black"
                when 1075 then "White"
                else "" end)                                                                                 as pupil_examination,
           (case sight_examination
                when 1065 then "Following Objects"
                when 1066 then "Not Following Objects"
                else "" end)                                                                                 as sight_examination,
           (case squint when 1065 then "Squint" when 1066 then "No Squint" else "" end)                      as squint,
           (case deworming_drug
                when 79413 then "Mebendazole"
                when 70439 then "Albendazole"
                else "" end)                                                                                 as deworming_drug,
           dosage,
           unit,
           ...
           (case counselled_on
                when 1914 then "HIV"
                when 1380 then "Nutrition"
                else "" end)                                                                                 as counselled_on,
           (case mnps_supplementation
                when 161649 then "Yes"
                when 1107 then "No"
                else "" end)                                                                                 as MNPS_Supplementation,
           (case LLIN when 1065 then "Yes" when 1066 then "No" else "" end)                                  as LLIN,
           comments,
           next_appointment_date
    from kenyaemr_etl.etl_hei_follow_up_visit;

    ALTER TABLE kenyaemr_datatools.hei_follow_up_visit
        ADD FOREIGN KEY (patient_id) REFERENCES kenyaemr_datatools.patient_demographics (patient_id);

    ALTER TABLE kenyaemr_datatools.hei_follow_up_visit
        ADD INDEX (visit_date);
    ALTER TABLE kenyaemr_datatools.hei_follow_up_visit
        ADD INDEX (infant_feeding);
    ...
    ALTER TABLE kenyaemr_datatools.hei_immunization
        ADD FOREIGN KEY (patient_id) REFERENCES kenyaemr_datatools.patient_demographics (patient_id);
    ...
    select patient_id,
           ...
           district,
           (case referred_by
                when 160539 then "VCT center"
                when 160631 then "HIV care clinic"
                when 160546 then "STI Clinic"
                when 161359 then "Home Based Care"
                when 160538 then "Antenatal/PMTCT Clinic"
                when 1725 then "Private Sector"
                when 1744 then "Chemist/pharmacist"
                when 160551 then "Self referral"
                when 1555 then "Community Health worker(CHW)"
                when 162050 then "CCC"
                when 164103 then "Diabetes Clinic"
                else "" end)                                                           as referred_by,
           referral_date,
           date_transferred_in,
           facility_transferred_from,
           district_transferred_from,
           ...
           (case relation_to_patient
                ...
                when 160639 then "Guardian"
                when 1527 then "Parent"
                when 5617 then "PARTNER OR SPOUSE"
                when 5622 then "Other"
                else "" end)                                                           as relation_to_patient,
           treatment_supporter_address,
           treatment_supporter_phone_contact,
           (case disease_classification
                when 42 then "Pulmonary TB"
                when 5042 then "Extra-Pulmonary TB"
                else "" end)                                                           as disease_classification,
           (case patient_classification
                when 159878 then "New"
                when 159877 then "Smear positive Relapse"
                when 159876 then "Smear negative Relapse"
                when 159874 then "Treatment after Failure"
                when 159873 then "Treatment resumed after defaulting"
                when 159872 then "Transfer in"
                when 163609 then "Previous treatment history unknown"
                else "" end)                                                           as patient_classification,
           (case pulmonary_smear_result
                when 703 then "Smear Positive"
                when 664 then "Smear Negative"
                when 1118 then "Smear not done"
                else "" end)                                                           as pulmonary_smear_result,
           (case has_extra_pulmonary_pleurial_effusion
                when 130059 then "Pleural effusion"
                else "" end)                                                           as has_extra_pulmonary_pleurial_effusion,
           (case has_extra_pulmonary_milliary when 115753 then "Milliary" else "" end) as has_extra_pulmonary_milliary,
           (case has_extra_pulmonary_lymph_node
                when 111953 then "Lymph nodes"
                else "" end)                                                           as has_extra_pulmonary_lymph_node,
           (case has_extra_pulmonary_menengitis
                when 111967 then "Meningitis"
                else "" end)                                                           as has_extra_pulmonary_menengitis,
    ...
    ALTER TABLE kenyaemr_datatools.tb_enrollment
        ADD FOREIGN KEY (patient_id) REFERENCES kenyaemr_datatools.patient_demographics (patient_id);
...
-- create table tb_follow_up_visit
    ...
    select patient_id,
           ...
           encounter_id,
           (case spatum_test
                when 160022 then "ZN Smear Microscopy"
                when 161880 then "Fluorescence Microscopy"
                else "" end)                                      as spatum_test,
           (case spatum_result
                when 159985 then "Scanty"
                when 1362 then "+"
                when 1363 then "++"
                when 1364 then "+++"
                when 664 then "Negative"
                else "" end)                                      as spatum_result,
           result_serial_number,
           quantity,
           date_test_done,
           (case bacterial_colonie_growth
                when 703 then "Growth"
                when 664 then "No growth"
                else "" end)                                      as bacterial_colonie_growth,
           number_of_colonies,
    ...
    ALTER TABLE kenyaemr_datatools.tb_follow_up_visit
        ADD FOREIGN KEY (patient_id) REFERENCES kenyaemr_datatools.patient_demographics (patient_id);

    ALTER TABLE kenyaemr_datatools.tb_follow_up_visit
        ADD INDEX (visit_date);
    ALTER TABLE kenyaemr_datatools.tb_follow_up_visit
        ADD INDEX (hiv_status);
...
-- create table tb_screening
    ...
    select patient_id,
           ...
           encounter_id,
           location_id,
           (case cough_for_2wks_or_more
                when 159799 then "Yes"
                when 1066 then "No"
                else "" end)                                                              as cough_for_2wks_or_more,
           (case confirmed_tb_contact
                when 124068 then "Yes"
                when 1066 then "No"
                else "" end)                                                              as confirmed_tb_contact,
           (case fever_for_2wks_or_more
                when 1494 then "Yes"
                when 1066 then "No"
                else "" end)                                                              as fever_for_2wks_or_more,
           (case noticeable_weight_loss
                when 832 then "Yes"
                when 1066 then "No"
                else "" end)                                                              as noticeable_weight_loss,
           (case night_sweat_for_2wks_or_more
                when 133027 then "Yes"
                when 1066 then "No"
                else "" end)                                                              as night_sweat_for_2wks_or_more,
           (case lethargy when 116334 then "Yes" else "" end)                             as lethargy,
           (case spatum_smear_ordered
                when 307 then "Yes"
                when 1066 then "No"
    ...
    ALTER TABLE kenyaemr_datatools.drug_event
        add FOREIGN KEY (patient_id) REFERENCES kenyaemr_datatools.patient_demographics (patient_id);
    ...
    ALTER TABLE kenyaemr_datatools.art_preparation
        ADD FOREIGN KEY (patient_id) REFERENCES kenyaemr_datatools.patient_demographics (patient_id);
    ALTER TABLE kenyaemr_datatools.art_preparation
        ADD INDEX (visit_date);
    ALTER TABLE kenyaemr_datatools.art_preparation
        ADD INDEX (ready_to_start_art);
    ...
    ALTER TABLE kenyaemr_datatools.enhanced_adherence
        ADD FOREIGN KEY (patient_id) REFERENCES kenyaemr_datatools.patient_demographics (patient_id);
    ALTER TABLE kenyaemr_datatools.enhanced_adherence
        ADD INDEX (visit_date);
    ...
    ALTER TABLE kenyaemr_datatools.triage
        ADD FOREIGN KEY (patient_id) REFERENCES kenyaemr_datatools.patient_demographics (patient_id);
    ALTER TABLE kenyaemr_datatools.triage
        ADD INDEX (visit_date);
    ...
    select id,
           ...
           (case living_with_patient
                when 1065 then "Yes"
                when 1066 then "No"
                when 162570 then "Declined to Answer"
                else "" end) as living_with_patient,
           (case pns_approach
                ...
                else "" end) as pns_approach,
           contact_listing_decline_reason,
           (case consented_contact_listing
                when 1065 then "Yes"
                when 1066 then "No"
                when 1067 then "Unknown"
                else "" end) as consented_contact_listing,
           voided
    from kenyaemr_etl.etl_patient_contact;
    ALTER TABLE kenyaemr_datatools.patient_contact
        ADD PRIMARY KEY (id);
    ALTER TABLE kenyaemr_datatools.patient_contact
        ADD FOREIGN KEY (patient_related_to) REFERENCES kenyaemr_datatools.patient_demographics (patient_id);
    ALTER TABLE kenyaemr_datatools.patient_contact
    ...
    ALTER TABLE kenyaemr_datatools.client_trace
        ADD FOREIGN KEY (client_id) REFERENCES kenyaemr_datatools.patient_contact (id);
    ALTER TABLE kenyaemr_datatools.client_trace
        ADD INDEX (date_created);
    ...
    ALTER TABLE kenyaemr_datatools.hts_test
        ADD FOREIGN KEY (patient_id) REFERENCES kenyaemr_datatools.patient_demographics (patient_id);
    ALTER TABLE kenyaemr_datatools.hts_test
        ADD INDEX (visit_date);
    ALTER TABLE kenyaemr_datatools.hts_test
    ...
    from kenyaemr_etl.etl_hts_referral_and_linkage l
             inner join kenyaemr_etl.etl_patient_demographics d on d.patient_id = l.patient_id and d.voided = 0;
    ALTER TABLE kenyaemr_datatools.hts_referral_and_linkage
        ADD FOREIGN KEY (patient_id) REFERENCES kenyaemr_datatools.patient_demographics (patient_id);
    ALTER TABLE kenyaemr_datatools.hts_referral_and_linkage
        ADD index (visit_date);
    ...
    as
    select r.*
    from kenyaemr_etl.etl_hts_referral r
             inner join kenyaemr_etl.etl_patient_demographics d on d.patient_id = r.patient_id and d.voided = 0;

    create table kenyaemr_datatools.current_in_care as
    select * from kenyaemr_etl.etl_current_in_care;
    ALTER TABLE kenyaemr_datatools.current_in_care
        add FOREIGN KEY (patient_id) REFERENCES kenyaemr_datatools.patient_demographics (patient_id);

    create table kenyaemr_datatools.ipt_followup as
    select * from kenyaemr_etl.etl_ipt_follow_up;
    alter table kenyaemr_datatools.ipt_followup
        add FOREIGN KEY (patient_id) REFERENCES kenyaemr_datatools.patient_demographics (patient_id);

    CREATE TABLE kenyaemr_datatools.default_facility_info as
    SELECT * from kenyaemr_etl.etl_default_facility_info;
    CREATE TABLE kenyaemr_datatools.person_address as
    SELECT * from kenyaemr_etl.etl_person_address;
    ...
    select patient_id,
           ...
           (case smoking_frequency
                ...
                when 163200 then 'Unknown if ever smoked' end) as smoking_frequency,
           (case drugs_use_frequency
                when 1090 then 'Never'
                when 1091 then 'Monthly or less'
                when 1092 then '2 to 4 times a month'
                when 1093 then '2 to 3 times a week'
                when 1094 then '4 or More Times a Week' end)   as drugs_use_frequency,
           date_created,
           date_last_modified,
           voided
    from kenyaemr_etl.etl_alcohol_drug_abuse_screening;

    ALTER TABLE kenyaemr_datatools.alcohol_drug_abuse_screening
        ADD FOREIGN KEY (patient_id) REFERENCES kenyaemr_datatools.patient_demographics (patient_id);
    ALTER TABLE kenyaemr_datatools.alcohol_drug_abuse_screening
        ADD INDEX (visit_date);
...
-- create table gbv_screening
    ...
    ALTER TABLE kenyaemr_datatools.gbv_screening
        ADD FOREIGN KEY (patient_id) REFERENCES kenyaemr_datatools.patient_demographics (patient_id);
    ALTER TABLE kenyaemr_datatools.gbv_screening
        ADD INDEX (visit_date);
...
-- create table gbv_screening
    ...
    ALTER TABLE kenyaemr_datatools.gbv_screening_action
        ADD FOREIGN KEY (patient_id) REFERENCES kenyaemr_datatools.patient_demographics (patient_id);
    ALTER TABLE kenyaemr_datatools.gbv_screening_action
        ADD INDEX (visit_date);
...
-- create table depression_screening
    ...
    ALTER TABLE kenyaemr_datatools.depression_screening
        ADD FOREIGN KEY (patient_id) REFERENCES kenyaemr_datatools.patient_demographics (patient_id);
    ALTER TABLE kenyaemr_datatools.depression_screening
        ADD INDEX (visit_date);
...
-- create table adverse_events
    ...
    select uuid,
           ...
           (case cause
                ...
                when 5622 then 'Other' end)   as cause,
           (case adverse_event
                when 1067 then 'Unknown'
                ...
                when 512 then 'Rash'
                when 114403 then 'Pain'
                when 147241 then 'Bleeding'
                when 135693 then 'Anaesthetic Reaction'
                when 167126 then 'Excessive skin removed'
                when 156911 then 'Damage to the penis'
                when 152045 then 'Problems with appearance'
                when 156567 then 'Hematoma/Swelling'
                when 139510 then 'Infection/Swelling'
                when 118771 then 'Difficulty or pain when urinating'
                when 163799 then 'Wound disruption (without signs of hematoma or infection)'
                when 5622 then 'Other' end)   as adverse_event,
           (case severity
                when 1498 then 'Mild'
                when 1499 then 'Moderate'
                when 1500 then 'Severe'
                when 162819 then 'Fatal'
                when 1067 then 'Unknown' end) as severity,
           start_date,
           (case action_taken
                ...
                when 1107 then 'NONE'
                when 1260 then 'STOP'
                when 5622 then 'Other' end)   as action_taken,
           date_created,
           date_last_modified,
           voided
    from kenyaemr_etl.etl_adverse_events;

    ALTER TABLE kenyaemr_datatools.adverse_events
        ADD FOREIGN KEY (patient_id) REFERENCES kenyaemr_datatools.patient_demographics (patient_id);
    ALTER TABLE kenyaemr_datatools.adverse_events
        ADD INDEX (visit_date);
...
-- create table allergies_chronic_illnesses
    ...
    select uuid,
           ...
           case is_chronic_illness_controlled
               when 1065 then 'Yes'
               when 1066
                   then 'No' end              as is_chronic_illness_controlled,
           (case allergy_causative_agent
           ...
           (case allergy_severity
                ...
                when 1067 then 'Unknown' end) as allergy_severity,
           allergy_onset_date,
           voided,
           date_created,
           date_last_modified
    ...
    ALTER TABLE kenyaemr_datatools.allergy_chronic_illness
        ADD FOREIGN KEY (patient_id) REFERENCES kenyaemr_datatools.patient_demographics (patient_id);
    ALTER TABLE kenyaemr_datatools.allergy_chronic_illness
        ADD INDEX (visit_date);
...
-- create table ipt_screening
    ...
    select uuid,
           ...
           (case upper_rightQ_abdomen_tenderness
                when 124994 then 'Yes'
                when 1066
                    then 'No' end)                                                      as upper_rightQ_abdomen_tenderness,
           date_created,
           date_last_modified,
           voided
    from kenyaemr_etl.etl_ipt_screening;

    ALTER TABLE kenyaemr_datatools.ipt_screening
        ADD FOREIGN KEY (patient_id) REFERENCES kenyaemr_datatools.patient_demographics (patient_id);
    ALTER TABLE kenyaemr_datatools.ipt_screening
        ADD INDEX (visit_date);
...
-- create table pre_hiv_enrollment_ART
    ...
    ALTER TABLE kenyaemr_datatools.pre_hiv_enrollment_art
        ADD FOREIGN KEY (patient_id) REFERENCES kenyaemr_datatools.patient_demographics (patient_id);
    ALTER TABLE kenyaemr_datatools.pre_hiv_enrollment_art
        ADD INDEX (visit_date);
...
-- create table covid_19_assessment
    ...
    select uuid,
           ...
           (case first_vaccine_type
                ...
                when
                    166379 then 'Sinopharm'
                when 1067 then 'Unknown'
                when 5622
                    then 'Other' end)                                                as first_vaccine_type,
           (case second_vaccine_type
                when 166156 then 'Astrazeneca'
                when 166355 then 'Johnson and Johnson'
                when 166154 then 'Moderna'
                when 166155 then 'Pfizer'
                ...
                when 1067 then 'Unknown'
                when 5622
                    then 'Other(Specify)' end)                                       as second_vaccine_type,
           first_dose,
           second_dose,
           first_dose_date,
           second_dose_date,
           ...
           (case booster_vaccine_taken
                ...
                when 1067 then 'Unknown'
                when 5622
                    then 'Other(Specify)' end)                                       as booster_vaccine_taken,
           date_taken_booster_vaccine,
           booster_sequence,
           (case booster_dose_verified when 164134 then 'Yes' end)                   as booster_dose_verified,
           (case ever_tested_covid_19_positive
                when 703 then 'Yes'
                when 664 then 'No'
                when 1067
                    then 'Unknown' end)                                              as ever_tested_covid_19_positive,
    ...
    ALTER TABLE kenyaemr_datatools.covid_19_assessment
        ADD FOREIGN KEY (patient_id) REFERENCES kenyaemr_datatools.patient_demographics (patient_id);
    ALTER TABLE kenyaemr_datatools.covid_19_assessment
        ADD INDEX (visit_date);
...
-- Create table prep_enrolment
    ...
    select uuid,
           ...
           case kp_type
               ...
               when 160579 then 'FSW' end                        as kp_type,
    ...
    ALTER TABLE kenyaemr_datatools.prep_enrolment
        ADD FOREIGN KEY (patient_id) REFERENCES kenyaemr_datatools.patient_demographics (patient_id);
    ALTER TABLE kenyaemr_datatools.prep_enrolment
        ADD INDEX (visit_date);
...
-- Create table cervical_cancer_screening
    ...
    ALTER TABLE kenyaemr_datatools.cervical_cancer_screening
        ADD FOREIGN KEY (patient_id) REFERENCES kenyaemr_datatools.patient_demographics (patient_id);
    ALTER TABLE kenyaemr_datatools.cervical_cancer_screening
        ADD INDEX (visit_date);
...
-- Create table contact
    ...
    ALTER TABLE kenyaemr_datatools.kp_contact
        ADD FOREIGN KEY (client_id) REFERENCES kenyaemr_datatools.patient_demographics (patient_id);
    ALTER TABLE kenyaemr_datatools.kp_contact
        ADD INDEX (visit_date);
...
-- Create table kp_client_enrollment
    ...
    ALTER TABLE kenyaemr_datatools.kp_client_enrollment
        ADD FOREIGN KEY (client_id) REFERENCES kenyaemr_datatools.patient_demographics (patient_id);
    ALTER TABLE kenyaemr_datatools.kp_client_enrollment
        ADD INDEX (visit_date);
...
-- Create table kp_clinical_visit
    ...
    ALTER TABLE kenyaemr_datatools.kp_clinical_visit
        ADD FOREIGN KEY (client_id) REFERENCES kenyaemr_datatools.patient_demographics (patient_id);
    ALTER TABLE kenyaemr_datatools.kp_clinical_visit
        ADD INDEX (visit_date);
...
-- Create table kp_peer_calendar
    ...
    ALTER TABLE kenyaemr_datatools.kp_peer_calendar
        ADD FOREIGN KEY (client_id) REFERENCES kenyaemr_datatools.patient_demographics (patient_id);
    ALTER TABLE kenyaemr_datatools.kp_peer_calendar
        ADD INDEX (visit_date);
...
-- Create table kp_sti_treatment
    ...
    ALTER TABLE kenyaemr_datatools.kp_sti_treatment
        ADD FOREIGN KEY (client_id) REFERENCES kenyaemr_datatools.patient_demographics (patient_id);
    ALTER TABLE kenyaemr_datatools.kp_sti_treatment
        ADD INDEX (visit_date);
...
-- Create table kp_peer_tracking
    ...
    ALTER TABLE kenyaemr_datatools.kp_peer_tracking
        ADD FOREIGN KEY (client_id) REFERENCES kenyaemr_datatools.patient_demographics (patient_id);
    ALTER TABLE kenyaemr_datatools.kp_peer_tracking
        ADD INDEX (visit_date);
...
-- Create table kp_treatment_verification
    ...
    ALTER TABLE kenyaemr_datatools.kp_treatment_verification
        ADD FOREIGN KEY (client_id) REFERENCES kenyaemr_datatools.patient_demographics (patient_id);
    ALTER TABLE kenyaemr_datatools.kp_treatment_verification
        ADD INDEX (visit_date);
...
-- Create table vmmc_enrolment
    ...
    select uuid,
           ...
           (case source_of_vmmc_info
                ...
                when 5622 then 'Other' end) as source_of_vmmc_info,
    ...
    ALTER TABLE kenyaemr_datatools.vmmc_enrolment
        ADD FOREIGN KEY (patient_id) REFERENCES kenyaemr_datatools.patient_demographics (patient_id);
    ALTER TABLE kenyaemr_datatools.vmmc_enrolment
        ADD INDEX (visit_date);
...
-- Create table vmmc_enrolment
    ...
    select uuid,
           ...
           location_id,
           encounter_id,
           (case circumcision_method
                when 167119 then 'Conventional Surgical'
                when 167120 then 'Device Circumcision' end)                      as circumcision_method,
           (case surgical_circumcision_method
                when 167121 then 'Sleeve resection'
                when 167122 then 'Dorsal Slit'
                when 167123 then 'Forceps Guide'
                when 5622
                    then 'Other' end)                                            as surgical_circumcision_method,
           reason_circumcision_ineligible,
           (case circumcision_device
                when 167124 then 'Shangring'
                when 5622
                    then 'Other' end)                                            as circumcision_device,
           specific_other_device,
           device_size,
           lot_number,
           (case anaesthesia_used
                when 161914 then 'Local Anaesthesia'
                when 162797
                    then 'Topical Anaesthesia' end)                              as anaesthesia_used,
           ...
           clinician_name,
           (case clinician_cadre
                when 162591 then 'MO'
                when 162592 then 'CO'
                when 1577
                    then 'Nurse' end)                                            as clinician_cadre,
           assist_clinician_name,
           (case assist_clinician_cadre
                when 162591 then 'MO'
                when 162592 then 'CO'
                when 1577
                    then 'Nurse' end)                                            as assist_clinician_cadre,
           theatre_number,
           date_created,
           date_last_modified,
           voided
    ...
    ALTER TABLE kenyaemr_datatools.vmmc_circumcision_procedure
        ADD FOREIGN KEY (patient_id) REFERENCES kenyaemr_datatools.patient_demographics (patient_id);
    ALTER TABLE kenyaemr_datatools.vmmc_circumcision_procedure
        ADD INDEX (visit_date);
...
-- Create table vmmc_medical_history
    ...
    select uuid,
           ...
           case current_regimen
               ...
               when 162562 then 'ABC/LPV/R/TDF'
               when 162559
                   then 'ABC/DDI/LPV/r' end                                                               as bcurrent_regimen,
           ccc_number,
           next_appointment_date,
           case hiv_care_facility
               when 163266 then 'This health facility'
               when 164407
                   then 'Other health facility' end                                                       as hiv_care_facility,
           ...
           case ever_had_surgical_operation
               when 1065 then 'Yes'
               when 1066
                   then 'No' end                                                                          as ever_had_surgical_operation,
           ...
           case ever_received_tetanus_booster
               when 1065 then 'Yes'
               when 1066
                   then 'No' end                                                                          as ever_received_tetanus_booster,
           ...
           reason_ineligible,
           case circumcision_method_chosen
               when 167119 then 'Conventional Surgical'
               when 167120
                   then 'Device Circumcision' end                                                         as circumcision_method_chosen,
           case conventional_method_chosen
               when 167121 then 'Sleeve resection'
               when 167122 then 'Dorsal Slit'
               when 167123 then 'Forceps Guide'
               when 5622
                   then 'Other' end                                                                       as conventional_method_chosen,
           device_name,
           device_size,
    ...
    ALTER TABLE kenyaemr_datatools.vmmc_medical_history
        ADD FOREIGN KEY (patient_id) REFERENCES kenyaemr_datatools.patient_demographics (patient_id);
    ALTER TABLE kenyaemr_datatools.vmmc_medical_history
        ADD INDEX (visit_date);
...
-- Create table vmmc_post_operation_assessment
    ...
    select provider,
           ...
           case given_post_procedure_instruction
               when 1065 then 'Yes'
               when 1066
                   then 'No' end                                                             as given_post_procedure_instruction,
    ...
    ALTER TABLE kenyaemr_datatools.vmmc_post_operation_assessment
        ADD FOREIGN KEY (patient_id) REFERENCES kenyaemr_datatools.patient_demographics (patient_id);
    ALTER TABLE kenyaemr_datatools.vmmc_post_operation_assessment
        ADD INDEX (visit_date);
    ...
    select patient_id,
           ...
           case population_type
               when 164928 then 'General Population'
               when 164929 then 'Key Population'
               when 138643 then 'Priority Population' end                            as population_type,
           key_population_type,
           priority_population_type,
           case department
           ...
           case patient_type
               ...
               when 164953
                   then 'NP:Non-Hospital Patient' end                                as patient_type,
           (case test_strategy
                when 164163 then "HP: Hospital Patient Testing"
                when 164953 then "NP: HTS for non-patients"
                when 164954 then "VI:Integrated VCT Center"
           ...
           case mother_hiv_status
               ...
               when 664 then 'Negative'
               when 1067
                   then 'Unknown' end                                                as mother_hiv_status,
           case tested_hiv_before when 1065 then 'Yes' when 1066 then 'No' end       as tested_hiv_before,
           case who_performed_test
               when 5619 then 'HTS Provider'
               when 164952
                   then 'Self Tested' end                                            as who_performed_test,
           (case test_results
                when 703 then 'Positive'
                when 664 then 'Negative'
                when 1067 then 'Unknown'
                else '' end)                                                         as test_results,
           date_tested,
           ...
           case alcohol_sex
               ...
               when 165027
                   then 'Always' end                                                 as alcohol_sex,
           money_sex,
           condom_burst,
           ...
           case tb_status
               ...
               when 142177 then 'Presumed TB'
               when 1662
                   then 'TB Confirmed' end                                           as tb_status,
           shared_needle,
    ...
    ALTER TABLE kenyaemr_datatools.hts_eligibility_screening
        ADD FOREIGN KEY (patient_id) REFERENCES kenyaemr_datatools.patient_demographics (patient_id);
    ALTER TABLE kenyaemr_datatools.hts_eligibility_screening
    ...
    ALTER TABLE kenyaemr_datatools.hts_eligibility_screening
        ADD INDEX (population_type);
    ALTER TABLE kenyaemr_datatools.hts_eligibility_screening
        ADD INDEX (eligible_for_test);

-- Creating drug_orders table --
    create table kenyaemr_datatools.drug_order as
    select * from kenyaemr_etl.etl_drug_order;
    ALTER TABLE kenyaemr_datatools.drug_order
        add FOREIGN KEY (patient_id) REFERENCES kenyaemr_datatools.patient_demographics (patient_id);
...
-- create table preventive_services
    ...
    ALTER TABLE kenyaemr_datatools.preventive_services
        ADD FOREIGN KEY (patient_id) REFERENCES kenyaemr_datatools.patient_demographics (patient_id);
    ALTER TABLE kenyaemr_datatools.preventive_services
        ADD INDEX (patient_id);
    ALTER TABLE kenyaemr_datatools.preventive_services
...
-- create table preventive_services
    ...
    ALTER TABLE kenyaemr_datatools.overdose_reporting
    ADD FOREIGN KEY (client_id) REFERENCES kenyaemr_datatools.patient_demographics (patient_id);
